...
async function getCityData(countryCode: string, stateSlug: string, citySlug: string) {
    ...
    // Get all insurance types that have pages for this city
    ...
    // Get nearby cities in same state
        ...
        <div className="min-h-screen bg-white">
                ...
                <div className="container mx-auto px-4">
                    <div className="flex items-center gap-2 text-blue-400 mb-4">
                        ...
                        <span className="text-sm font-medium uppercase tracking-wider">
                            <Link href={`/states/${country.code}/${state.slug}`} className="hover:text-blue-300">{state.name}</Link>
                        </span>
                    </div>
                ...
                <div className="container mx-auto px-4">
                    ...
                    {availableInsuranceTypes.length > 0 ? (
                            ...
                            {availableInsuranceTypes.map((type: any) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-slate-500">in {city.name}</p>
                                    </div>
                        ...
                        <div className="text-center py-12 bg-slate-50 rounded-lg mb-12">
                            ...
                            <p className="text-slate-500 mb-6">We're building guides for {city.name}. Check out {state.name} in the meantime.</p>
                            <Link href={`/states/${country.code}/${state.slug}`} className="inline-flex items-center gap-2 text-blue-600 font-semibold hover:underline">
                                View {state.name} <ArrowRight className="w-4 h-4" />
                            </Link>
                        </div>
                    )}
            ...
            {nearbyCities.length > 0 && (
                <section className="py-16 bg-slate-50 border-y border-slate-200">
                    <div className="container mx-auto px-4">
                        <h2 className="text-2xl font-bold text-slate-900 mb-8">Other Cities in {state.name}</h2>
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                            {nearbyCities.map((c: any) => (
                                <Link
                                    key={c.id}
                                    href={`/states/${country.code}/${state.slug}/${c.slug}`}
                                    className="p-3 bg-white rounded-lg border border-slate-200 hover:border-blue-500 hover:shadow-sm transition-all text-slate-700 font-medium text-sm"
                                >
                                    {c.name}
                                </Link>
                            ))}
