...
import {
    ...
    AlertTriangle, Umbrella, Droplets, Wind, CloudRain, Percent
} from 'lucide-react';
                    ...
                    <div className="container mx-auto px-4">
                        ...
                        <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
                            {relatedLinks.nearbyCities?.length > 0 && (
                                <div>
                                    <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2"><MapPin size={18} className="text-orange-600" />Nearby Cities</h3>
                                    <ul className="space-y-2">
                                        {relatedLinks.nearbyCities.slice(0, 8).map((link: any, i: number) => (
                                            <li key={i}><Link href={link.href} className="text-slate-600 hover:text-orange-600 text-sm flex items-center gap-1"><ChevronDown size={14} className="rotate-[-90deg]" />{link.label}</Link></li>
                                        ))}
                                    </ul>
                                </div>
                            )}
                            {relatedLinks.otherNiches?.length > 0 && (
                                <div>
                                    <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2"><Shield size={18} className="text-orange-600" />Other Insurance</h3>
                                    <ul className="space-y-2">
                                        {relatedLinks.otherNiches.slice(0, 6).map((link: any, i: number) => (
                                            <li key={i}><Link href={link.href} className="text-slate-600 hover:text-orange-600 text-sm flex items-center gap-2"><span>{link.icon}</span> {link.label}</Link></li>
                                        ))}
                                    </ul>
                                </div>
                            )}
