...
import { Shield, ArrowRight, MapPin, FileText, Car, Home, Heart, Stethoscope, Dog, Briefcase, Bike, Plane, Ship, Umbrella } from 'lucide-react';
...
async function getStateData(countryCode: string, stateSlug: string) {
    // Get insurance types for header
    ...
    // Get all insurance types that have pages for this state
    ...
    // Get some nearby states for header
        ...
        <div className="min-h-screen bg-white">
            ...
            <section className="bg-[#0F172A] py-16">
                <div className="container mx-auto px-4">
                    <div className="flex items-center gap-2 text-blue-400 mb-4">
                        ...
                        <span className="text-sm font-medium uppercase tracking-wider">{country.name}</span>
                    </div>
            ...
            <section className="py-16">
                <div className="container mx-auto px-4">
                    ...
                    {availableInsuranceTypes.length > 0 ? (
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 mb-12">
                            {availableInsuranceTypes.map((type: any) => (
                                <Link
                                    ...
                                    className="group flex items-center gap-4 p-6 bg-white rounded-lg border border-slate-200 hover:border-blue-500 hover:shadow-lg transition-all"
                                >
                                    <div className="w-14 h-14 bg-blue-50 rounded-xl flex items-center justify-center group-hover:bg-blue-100 transition-colors">
                                        {getIconForType(type.slug, "w-7 h-7 text-blue-600")}
                                    </div>
                                    <div className="flex-1">
                                        <h3 className="font-bold text-slate-900 group-hover:text-blue-600 transition-colors">
                                            {type.name}
                                        </h3>
                                        <p className="text-sm text-slate-500">in {state.name}</p>
                                    </div>
                        ...
                        <div className="text-center py-12 bg-slate-50 rounded-lg mb-12">
                            <FileText className="w-12 h-12 text-slate-300 mx-auto mb-4" />
                            <h3 className="text-xl font-bold text-slate-900 mb-2">Content Coming Soon</h3>
                            <p className="text-slate-500 mb-6">We're building guides for {state.name}. Browse other states in the meantime.</p>
                            <Link href="/states" className="inline-flex items-center gap-2 text-blue-600 font-semibold hover:underline">
                    ...
                    <div className="border-t pt-12">
                        <h3 className="text-lg font-bold text-slate-900 mb-6">Browse All Insurance Types</h3>
                            ...
                            {insuranceTypes.map((type: any) => {
                                ...
                                return (
                                    <Link
                                        key={type.id}
                                        href={`/${type.slug}/${country.code}/${state.slug}`}
                                        className={`flex items-center gap-2 p-3 rounded-lg border transition-all ${hasPages
                                    ...
                                    >
                                        {getIconForType(type.slug, "w-5 h-5 text-slate-500")}
                                        <span className="text-sm font-medium text-slate-700 truncate">{type.name}</span>
                                    </Link>
                                );
                            })}
                        </div>
            ...
            {cities.length > 0 && (
                <section className="py-16 bg-slate-50 border-t border-slate-200">
                    <div className="container mx-auto px-4">
                        <h2 className="text-xl font-bold text-slate-900 mb-8">Cities in {state.name}</h2>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3 mb-10">
                            {cities.filter((c: any) => c.isMajorCity).map((city: any) => (
                                <Link
                                    ...
                                    className="p-3 bg-white rounded-lg border border-slate-200 hover:border-blue-500 hover:shadow-sm transition-all text-slate-900 font-bold text-sm flex justify-between items-center"
                                >
                                    {city.name}
                        ...
                        {/* All Cities List */}
                        <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4">All Cities</h3>
