...
interface LeadCaptureFormProps {
    variant?: 'inline' | 'stacked' | 'compact' | 'hero' | 'sidebar' | 'minimal';
...
export default function LeadCaptureForm({
    ...
    showEmail = false, // Default FALSE - MarketCall will collect email
    showPhone = false,
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            // Try to get MarketCall affiliate
            ...
            if (data.success && data.redirectUrl) {
                ...
                // Redirect to MarketCall
                window.location.href = data.redirectUrl;
                return;
            }
        } catch (error) {
            console.log('No MarketCall affiliate found, falling back');
        }
    ...
    }, [email, phone, insuranceType, state, source, redirectUrl, buttonText, router, showEmail, showPhone]);

    // Minimal variant - just a button, no form fields
    if (variant === 'minimal') {
        return (
            <div className={className}>
                <button
                    onClick={() => {
                        ...
                        fetch(`/api/redirect-offer?${params.toString()}`)
                            .then(r => r.json())
                            .then(data => {
                                if (data.success && data.redirectUrl) {
                                    window.location.href = data.redirectUrl;
                    ...
                    disabled={isLoading}
                    className={`${colors.button} text-white font-bold px-8 py-4 rounded-xl transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg hover:shadow-xl ${className}`}
                >
                    {isLoading ? (
    ...
    // Hero variant - email only (optional)
    if (variant === 'hero') {
        return (
    ...
    // Sidebar variant - email/phone optional
    if (variant === 'sidebar') {
        return (
    ...
    // Default: minimal button with optional fields
    return (
        <form onSubmit={handleSubmit} className={`${className}`}>
            <div className="flex flex-col gap-3">
                {showEmail && (
                    <div className="relative">
                        ...
                        <input
                            ...
                            value={email}
                            onChange={(e) => setEmail(e.target.value)}
                            className={`w-full pl-12 pr-4 py-3.5 text-slate-900 placeholder-slate-400 focus:outline-none text-lg rounded-xl border ${errors.email ? 'bg-red-50 border-red-500' : 'bg-slate-50 border-slate-200'}`}
                        />
                    </div>
                )}
                ...
                <button
                    type="submit"
                    disabled={isLoading}
                    className={`${colors.button} text-white font-semibold px-8 py-4 rounded-xl transition-all disabled:opacity-50 flex items-center justify-center gap-2`}
                >
                    {isLoading ? (
                        <>
                            <Loader2 className="animate-spin" size={22} />
